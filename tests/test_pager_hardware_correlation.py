...
def test_pager_correlation_extract_matches_committed() -> None:
    ...
    assert mod.PROFILE_PATH.is_file(), f"Assumed profile missing at {mod.PROFILE_PATH}"
...
def test_bench_correlation_statistical_accuracy() -> None:
    ...
    assert corr.supports_measured_evidence is False
    assert corr.metadata["evidence_class"] == "assumed"


def test_representative_profile_cannot_claim_measured() -> None:
    """Representative values stay assumed even when numerical fit is good."""
    assert mod.PROFILE_PATH.is_file()
    data = json.loads(mod.PROFILE_PATH.read_text(encoding="utf-8"))
    assert data["evidence_class"] == "assumed"
    assert data["status"] == "SENSITIVITY_ONLY"
...
def test_imported_measurement_requires_instrument_metadata(tmp_path: Path) -> None:
    measurement = tmp_path / "measurement.json"
    measurement.write_text(
        json.dumps({"evidence_class": "measured", "measurements": [{"point_id": "P1"}]}),
        encoding="utf-8",
    )
    with pytest.raises(ValueError, match="measurement provenance is incomplete"):
        load_bench_measurements(measurement)
