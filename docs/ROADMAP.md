...
- [x] Freeze tile-level validation report: `verification/reports/tile-v1-r5-validation-summary.{json,md,svg}` deterministically cross-checks the three-profile tile configuration, 10-case/30-output SPICE regression, profile-driven calibration, and partial-sum formulas. The frozen verdict is honestly `NOT_MET` / `SYSTEM_SIMULATED`, with reproducibility, source-divergence, invalid-artifact, formula, diagram, and claim-level tests in `tests/test_tile_v1_report.py`.
- [ ] Apply required `crossbar-v1` non-idealities in the tile/accelerator path with per-mechanism attribution: the frozen report proves `CrossbarTile` currently consumes only `g0_s`/`gscale_s_per_w`; IR drop (`mvm_error_16x16_1ohm_pct`), programming/read variation (`sigma_prog_rel`, `sigma_read_rel`), drift, stuck-at faults, and I-V non-linearity remain unconsumed. Keep `physical_claim=False` until assumed inputs are replaced or the claim is narrowed, and add held-out/corner calibration evidence before closing R5.

### Gate R5 exit

The tile simulator is a calibrated abstraction of the proposed circuit/device stack, not an independent collection of hand-chosen parameters. **Not yet met:** the frozen validation report records the unconsumed `crossbar-v1` mechanisms and same-sample `SYSTEM_SIMULATED` calibration as explicit blockers.
